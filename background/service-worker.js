...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "START_CRAWL") {
    ...
    sendResponse({
      ...
      // Sending the 'current' URL might be tricky if processQueue is waiting,
      // but we can track the last picked URL.
  ...
  while (crawlQueue.length > 0 && isCrawling) {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `Rendered HTML fetched in ${renderMs}ms (len=${htmlText.length})`
        );
      } catch (e) {
      ...
      if (!htmlText) {
        ...
        console.log(
          `Raw HTML fetched in ${fetchMs}ms (len=${htmlText.length})`
        );
      }
      ...
      func: async (inputText) => {
        try {
          ...
          const session = await LanguageModel.create({
            systemPrompt:
              "You are an expert technical writer. You improve Markdown documentation.",
            expectedInputs: [{ type: "text", languages: ["en", "ja"] }],
            expectedOutputs: [{ type: "text", languages: ["ja"] }],
          });
