...
* ニュースサイトの特定トピックに関する記事を収集し、要約を作成する
...
* **AI整形 (Gemini Nano)**:
  ...
  * 複数ページに共通する定型文（ヘッダー/フッターの残りなど）の削除・統合。

### 3.3 要約・統合機能 (AI)

* **セクション要約**: 各ページのセクションごとに短い要約を生成するオプション。
* **全体要約**: 取得した全ページの内容に基づくエグゼクティブサマリーをドキュメント冒頭に生成するオプション。
* **Recursive Summarization**: 長文コンテンツの場合、分割して要約を行い、最終的に統合する。
