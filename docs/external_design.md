...
本ドキュメントは、AI Markdown Crawlerのユーザーインターフェース（UI）および外部インターフェース（出力形式）の詳細を定義する。
...
3. **AI Options (AI機能設定)**
    ...
    * **項目**:
        ...
        * `[ ] Merge Duplicates`: 複数ページの共通部分（フッター等）を統合・排除する。
        * `[ ] Generate Summary`: 各ページおよび全体の要約を生成する。
...
* デフォルト: `[Domain]_[PageTitle]_[Date].md`
  * 例: `example_com_Documentation_2025-01-03.md`
...
```markdown
# Executive Summary
(AIオプション「Generate Summary」有効時のみ)
全ページの総括要約がここに記載される。
